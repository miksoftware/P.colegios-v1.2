<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Acuerdos (Gastos) - Contraloría</h1>
                <p class="text-gray-500 mt-1">Movimientos presupuestales de gasto: adiciones, reducciones, créditos y contracréditos</p>
            </div>
            @can('reports.export')
            <button id="btn-export-agreements"
        ...
        @php
            ...
            $totalCredito       = collect($rows)->sum('credito');
            $totalContracredito = collect($rows)->sum('contracredito');
        @endphp
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                <p class="text-xs text-gray-500">Total Adiciones</p>
                <p class="text-xl font-bold text-green-600 mt-1">${{ number_format($totalAdicion, 0, ',', '.') }}</p>
            </div>
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                <p class="text-xs text-gray-500">Total Reducciones</p>
                <p class="text-xl font-bold text-red-600 mt-1">${{ number_format($totalReduccion, 0, ',', '.') }}</p>
            </div>
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                <p class="text-xs text-gray-500">Total Créditos</p>
                <p class="text-xl font-bold text-blue-600 mt-1">${{ number_format($totalCredito, 0, ',', '.') }}</p>
            </div>
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                <p class="text-xs text-gray-500">Total Contracréditos</p>
                <p class="text-xl font-bold text-orange-600 mt-1">${{ number_format($totalContracredito, 0, ',', '.') }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <table class="min-w-full divide-y divide-gray-200 text-xs">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-3 py-3 text-right font-semibold text-blue-700 uppercase tracking-wider whitespace-nowrap">Crédito</th>
                            <th class="px-3 py-3 text-right font-semibold text-orange-700 uppercase tracking-wider whitespace-nowrap">Contracrédito</th>
                            <th class="px-3 py-3 text-right font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Aplazamiento</th>
                            <th class="px-3 py-3 text-right font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Desaplazamiento</th>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-3 py-2 text-right whitespace-nowrap {{ $row['credito'] > 0 ? 'font-semibold text-blue-600' : 'text-gray-400' }}">
                                {{ $row['credito'] > 0 ? '$'.number_format($row['credito'], 0, ',', '.') : '0' }}
                            </td>
                            <td class="px-3 py-2 text-right whitespace-nowrap {{ $row['contracredito'] > 0 ? 'font-semibold text-orange-600' : 'text-gray-400' }}">
                                {{ $row['contracredito'] > 0 ? '$'.number_format($row['contracredito'], 0, ',', '.') : '0' }}
                            </td>
                            <td class="px-3 py-2 text-right whitespace-nowrap text-gray-400">0</td>
                            <td class="px-3 py-2 text-right whitespace-nowrap text-gray-400">0</td>
                    ...
                    <tfoot class="bg-gray-50 border-t-2 border-gray-200">
                        <tr class="font-bold text-xs">
                            ...
                            <td class="px-3 py-3 text-right text-blue-700">${{ number_format($totalCredito, 0, ',', '.') }}</td>
                            <td class="px-3 py-3 text-right text-orange-700">${{ number_format($totalContracredito, 0, ',', '.') }}</td>
                            <td class="px-3 py-3 text-right text-gray-500">0</td>
                            <td class="px-3 py-3 text-right text-gray-500">0</td>
                            <td></td>
                        </tr>
                    </tfoot>
            ...
            <div class="text-center py-16 text-gray-400">
                ...
                <p class="text-sm">No hay movimientos presupuestales para la vigencia <strong>{{ $filterYear }}</strong>.</p>
            </div>
            @endif
        </div>
    ...
    {{-- Datos ocultos para exportación Excel --}}
    <div id="agreements-data" class="hidden"
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const btn = document.getElementById('btn-export-agreements');
        if (!btn) return;

        btn.addEventListener('click', function () {
            const el     = document.getElementById('agreements-data');
            ...
            // Formato numérico para columnas D–I (índices 3–8)
            const numFmt = '#,##0';
            ...
            ws['!cols'] = [
                ...
                { wch: 15 }, // reducción
                { wch: 15 }, // crédito
                { wch: 15 }, // contracrédito
                { wch: 15 }, // aplazamiento
                { wch: 15 }, // desaplazamiento
                { wch: 50 }, // motivo
            ];

            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, 'Acuerdos Gastos');

            const safeName = school.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 30);
            XLSX.writeFile(wb, `Contraloria_Acuerdos_${safeName}_${year}.xlsx`);
        });
    });
    </script>
    @endpush
