<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Pago de Proveedores - Contraloría</h1>
                <p class="text-gray-500 mt-1">Pagos con CDP y RP (contrato o pago directo con soporte presupuestal)</p>
            </div>
            @can('reports.export')
            <button id="btn-export-suppliers-payments"
        ...
        @php
            ...
            $totalDescRet    = collect($rows)->sum('desc_retenciones');
            $totalOtrosDesc  = collect($rows)->sum('otros_descuentos');
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                ...
                <p class="text-2xl font-bold text-red-600 mt-1">${{ number_format($totalDescRet + $totalOtrosDesc, 0, ',', '.') }}</p>
            </div>
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 text-xs" id="table-suppliers-payments">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-3 py-3 text-left font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Tipo De Pago</th>
                            ...
                            <th class="px-3 py-3 text-right font-semibold text-orange-600 uppercase tracking-wider whitespace-nowrap">Desc. Seg. Social</th>
                            <th class="px-3 py-3 text-right font-semibold text-red-600 uppercase tracking-wider whitespace-nowrap">Desc. Retenciones</th>
                            <th class="px-3 py-3 text-right font-semibold text-red-700 uppercase tracking-wider whitespace-nowrap">Otros Descuentos</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-3 py-2 whitespace-nowrap font-mono text-gray-800 font-medium">{{ $row['codigo_presup'] }}</td>
                            <td class="px-3 py-2 whitespace-nowrap text-gray-700">{{ $row['tipo_pago'] }}</td>
                            <td class="px-3 py-2 whitespace-nowrap text-center font-mono text-gray-700">{{ $row['no_comprobante'] }}</td>
                            <td class="px-3 py-2 text-gray-800 font-medium whitespace-nowrap">{{ $row['beneficiario'] }}</td>
                            ...
                            <td class="px-3 py-2 whitespace-nowrap text-right font-mono text-orange-600">${{ number_format($row['desc_seg_social'], 0, ',', '.') }}</td>
                            <td class="px-3 py-2 whitespace-nowrap text-right font-mono text-red-600">${{ number_format($row['desc_retenciones'], 0, ',', '.') }}</td>
                            <td class="px-3 py-2 whitespace-nowrap text-right font-mono text-red-700">${{ number_format($row['otros_descuentos'], 0, ',', '.') }}</td>
                    ...
                    <tfoot class="bg-gray-50 border-t-2 border-gray-300">
                        <tr>
                            <td colspan="7" class="px-3 py-3 font-bold text-gray-700 text-xs uppercase">Totales</td>
                            <td class="px-3 py-3 text-right font-bold text-blue-700 font-mono whitespace-nowrap">${{ number_format($totalValor, 0, ',', '.') }}</td>
                            <td class="px-3 py-3 text-right font-bold text-orange-600 font-mono whitespace-nowrap">$0</td>
                            <td class="px-3 py-3 text-right font-bold text-red-600 font-mono whitespace-nowrap">${{ number_format($totalDescRet, 0, ',', '.') }}</td>
                            <td class="px-3 py-3 text-right font-bold text-red-700 font-mono whitespace-nowrap">${{ number_format($totalOtrosDesc, 0, ',', '.') }}</td>
            ...
            <div class="flex flex-col items-center justify-center py-16 text-gray-400">
                ...
                <p class="text-sm mt-1">No se encontraron pagos de proveedores con CDP y RP para la vigencia {{ $filterYear }}.</p>
            </div>
            @endif
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    var btn = document.getElementById('btn-export-suppliers-payments');
    if (!btn) return;
    ...
    btn.addEventListener('click', function () {
        ...
        var totalDescRet   = rows.reduce(function (s, r) { return s + r.desc_retenciones; }, 0);
        var totalOtros     = rows.reduce(function (s, r) { return s + r.otros_descuentos; }, 0);
        var totalNeto      = rows.reduce(function (s, r) { return s + r.neto; }, 0);
        dataRows.push(['', '', '', '', '', '', 'TOTALES', totalValor, 0, totalDescRet, totalOtros, totalNeto, '', '', '']);
        ...
        ws['!cols'] = [
            ...
            { wch: 40 }, // Detalle
            { wch: 22 }, // Valor
            { wch: 18 }, // Desc. Seg. Social
            { wch: 20 }, // Desc. Retenciones
            { wch: 18 }, // Otros Descuentos
            { wch: 18 }, // Neto
            { wch: 20 }, // Banco
            { wch: 22 }, // No. Cuenta
            { wch: 18 }, // No. Cheque
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Pago Proveedores');
        XLSX.writeFile(wb, 'Contraloria_PagoProveedores_' + schoolName + '_' + year + '.xlsx');
    });
});
</script>
@endpush
