    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if($currentView === 'list')
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                ...
                @endcan
                @if(auth()->check() && auth()->user()->email === 'softwaremik@gmail.com')
                    <button wire:click="deleteAllDataForMikSoftware" onclick="confirm('¿Está seguro de eliminar TODO el proceso precontractual, contractual y postcontractual? Esta acción es irreversible.') || event.stopImmediatePropagation()" class="px-4 py-2.5 bg-red-600 text-white rounded-xl hover:bg-red-700 transition-colors flex items-center gap-2 shadow-sm">
                                ...
                                <tr class="hover:bg-gray-50" wire:key="conv-{{ $conv->id }}">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-1">
                                            ...
                                            @if($conv->status === 'draft' || (auth()->user()->isAdmin() && $conv->status === 'awarded' && !$conv->contract))
                    ...
                    <div class="flex flex-col lg:flex-row justify-between items-start gap-4">
                        ...
                        <div class="text-right">
                            ...
                            <div class="mt-4 flex flex-wrap gap-2 justify-end">
                                ...
                                {{-- Botón Eliminar (Solo Admin y si es la última) --}}
                                @if($isLastConvocatoria && (auth()->user()->is_system_admin || auth()->user()->hasRole('Admin')))
                                    <button wire:click="deleteConvocatoria" wire:confirm="¿Está seguro de ELIMINAR esta convocatoria? Esta acción borrará permanentemente la convocatoria y todos sus CDPs asociados, y reiniciará el consecutivo." class="px-3 py-1.5 bg-red-600 text-white text-sm rounded-lg hover:bg-red-700 inline-flex items-center gap-1">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                        Eliminar
                                    </button>
                                @endif
                        ...
                        <div class="flex gap-2">
                            ...
                            @if($convocatoria->status === 'evaluation')
                                @can('precontractual.evaluate')
                                    @if($convocatoria->evaluation_date)
                                        <span class="px-3 py-1.5 bg-green-100 text-green-700 text-sm rounded-lg flex items-center gap-1" title="Evaluada el {{ $convocatoria->evaluation_date->format('d/m/Y h:i A') }}">
                        ...
                        @if($convocatoria->evaluation_date)
                            <div class="mb-4 p-3 bg-green-50 border border-green-200 rounded-xl flex items-center gap-2">
                                ...
                                <span class="text-sm text-green-800">
                                    Evaluación realizada el <span class="font-semibold">{{ $convocatoria->evaluation_date->format('d/m/Y') }}</span> a las <span class="font-semibold">{{ $convocatoria->evaluation_date->format('h:i A') }}</span>
                                </span>
                            </div>
                        @endif
                ...
                <form wire:submit="saveEvaluation">
                    ...
                    <div class="p-6 max-h-[60vh] overflow-y-auto">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6 p-4 bg-yellow-50 rounded-xl border border-yellow-200">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Hora de Evaluación <span class="text-red-500">*</span></label>
                                <input type="time" wire:model="evaluationTime" class="w-full rounded-xl border-gray-300 focus:border-yellow-500 focus:ring-yellow-500">
                                @error('evaluationTime') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>
                        </div>
